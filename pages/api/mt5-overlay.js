...
const deviceAccess = createIndicatorDeviceAccess({
  getEnforcement: async () => {
    const { data, error } = await supabase.from('indicator_device_enforcement').select('mode,enabled').eq('product_code', MT5_OVERLAY_PRODUCT_CODE).maybeSingle();
    if (error) throw error;
  ...
  registerDevice: async (device) => {
    const { data, error } = await supabase.rpc('register_indicator_device', {
      ...
      p_product_code: MT5_OVERLAY_PRODUCT_CODE,
      p_platform: PLATFORM,
