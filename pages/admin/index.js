...
function EditUserModal({ user, onClose, onSaved }) {
  ...
  async function save(e) {
    e.preventDefault(); setLoading(true); setError('');
    const body = { id: user.id, ...form, expires_at: form.expires_at || null, maintenance_bypass: form.maintenance_bypass };
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-bright)', borderRadius: 12, padding: 28, width: 420, display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <form onSubmit={save} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: form.maintenance_bypass ? 'rgba(0,255,159,0.06)' : 'rgba(255,77,109,0.04)', border: `1px solid ${form.maintenance_bypass ? '#00ff9f33' : '#ff4d6d22'}`, borderRadius: 8, padding: '10px 14px' }}>
            <div>
              <div style={{ fontFamily: mono, fontSize: 9, letterSpacing: 2, color: '#445566' }}>MAINTENANCE BYPASS</div>
              <div style={{ fontFamily: raj, fontSize: 12, color: form.maintenance_bypass ? '#00ff9f' : '#ff4d6d', marginTop: 2 }}>{form.maintenance_bypass ? 'Can access site when offline' : 'Blocked when site is offline'}</div>
            </div>
            <button type="button" onClick={() => setForm(f => ({ ...f, maintenance_bypass: !f.maintenance_bypass }))} style={{
              background: form.maintenance_bypass ? 'rgba(0,255,159,0.15)' : 'rgba(255,77,109,0.08)',
              border: `1px solid ${form.maintenance_bypass ? '#00ff9f55' : '#ff4d6d44'}`,
              borderRadius: 20, width: 48, height: 24, position: 'relative', cursor: 'pointer', transition: 'all 0.2s', padding: 0, flexShrink: 0
            }}>
              <div style={{
                width: 18, height: 18, borderRadius: '50%',
                background: form.maintenance_bypass ? '#00ff9f' : '#ff4d6d',
                boxShadow: `0 0 8px ${form.maintenance_bypass ? '#00ff9f55' : '#ff4d6d55'}`,
                position: 'absolute', top: 2,
                left: form.maintenance_bypass ? 26 : 3,
                transition: 'all 0.2s'
              }} />
            </button>
          </div>
...
export default function AdminPanel() {
  ...
  async function toggleBypass(user) {
    await fetch('/api/admin/users', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: user.id, maintenance_bypass: !user.maintenance_bypass }) });
    loadUsers();
  }
        ...
        <div style={{ flex: 1, padding: '0 24px 24px', zIndex: 1, overflowX: 'auto' }}>
          {tab === 'USERS' && (
            <table style={{ width: '100%', borderCollapse: 'collapse', background: '#080c18', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
              ...
              <tbody>
                {users.length === 0
                  ? <tr><td colSpan={10} style={{ textAlign: 'center', padding: 40, fontFamily: mono, fontSize: 10, color: '#2a3550' }}>NO USERS YET · Click + CREATE USER</td></tr>
                  : users.map(u => {
                      ...
                      <tr key={u.id} style={{ borderBottom: '1px solid #111827', opacity: u.is_active ? 1 : 0.5 }}>
                        ...
                        <td style={tc}>
                          <button onClick={() => toggleBypass(u)} style={{
                            background: u.maintenance_bypass ? 'rgba(0,255,159,0.12)' : 'rgba(255,77,109,0.06)',
                            border: `1px solid ${u.maintenance_bypass ? '#00ff9f44' : '#ff4d6d33'}`,
                            borderRadius: 20, width: 44, height: 22, position: 'relative', cursor: 'pointer', transition: 'all 0.2s', padding: 0
                          }}>
                            <div style={{
                              width: 16, height: 16, borderRadius: '50%',
                              background: u.maintenance_bypass ? '#00ff9f' : '#ff4d6d',
                              boxShadow: `0 0 6px ${u.maintenance_bypass ? '#00ff9f55' : '#ff4d6d55'}`,
                              position: 'absolute', top: 2,
                              left: u.maintenance_bypass ? 24 : 3,
                              transition: 'all 0.2s'
                            }} />
                          </button>
                        </td>
