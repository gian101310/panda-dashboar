#ifndef PANDA_DASHBOARD_OVERLAY_MT4_CORE
#define PANDA_DASHBOARD_OVERLAY_MT4_CORE
...
private:
   ...
   void LoadDeviceCredentials()
   {
      ...
      if(StringLen(m_device_id) < 16)
      {
         ...
         m_device_id = "mt4-" + Digest(TerminalInfoString(TERMINAL_COMMONDATA_PATH) + IntegerToString((int)TimeLocal()) + IntegerToString(MathRand()))
   ...
   void RefreshFromNetwork()
   {
      ...
      ResetLastError();
      int status_code = WebRequest("GET", PANDA_MT4_ENDPOINT, headers, 12000, request_data, response_data, response_headers);
...
public:
   ...
   bool Initialize(const string edition, const string header_name, const string credential, const int corner = PANEL_BOTTOM_LEFT)
   {
      ...
      m_prefix = "PandaMT4." + IntegerToString((int)ChartID()) + (m_can_http ? ".F." : ".I.");
      DeleteByPrefix("PandaMT4.");
      m_cache_key = Digest("MT4|" + m_header_name + "|" + m_credential);
      m_cache_file = "PandaOverlay\\MT4-" + m_cache_key + ".json";
      FolderCreate("PandaOverlay", FILE_COMMON);
      m_device_file = "PandaOverlay\\MT4-device-" + Digest(m_credential) + ".txt";
      if(m_edition == "LICENSED") LoadDeviceCredentials();
      m_lock_key = "Panda.MT4.Lock." + m_cache_key;
      m_attempt_key = "Panda.MT4.Try." + m_cache_key;
      m_received_key = "Panda.MT4.Rx." + m_cache_key;
      m_position_key = "Panda.MT4.Pos." + edition + (m_can_http ? ".F." : ".I.") + IntegerToString((int)ChartID());
