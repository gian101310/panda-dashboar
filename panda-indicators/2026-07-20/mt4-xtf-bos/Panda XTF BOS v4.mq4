//+------------------------------------------------------------------+
//|  Panda XTF BOS v4                                                 |
//|  v4: SuperTrend + Follow lines OFF by default (calc still runs);     |
//|  clearer BOS/flip arrows + labels across history; SIGNAL surfaces    |
//|  EXECUTION (PULLBACK 5-8, MARKET >=9) and ready/trigger state.       |
...
int OnInit()
{
   ...
   IndicatorShortName("Panda XTF BOS v4");
...
void ProcessClosedShift(const int b, const int rates_total, const bool isLive)
{
   ...
   if(isLive)
   {
      ...
      if(buyReady && bosBull)
      {
         ...
         if(ShowTriggers) { DrawArrowMark("sig", Time[b], Low[b], 233, clrLime, false, 4); DrawSigText("sig", Time[b], Low[b], "BUY", clrLime, false); }
         FireAlert("BUY");
      }
      else if(sellReady && bosBear)
      {
         SignalStatus = "SELL TRIGGER (" + exec + ")";
         if(ShowTriggers) { DrawArrowMark("sig", Time[b], High[b], 234, clrRed, true, 4); DrawSigText("sig", Time[b], High[b], "SELL", clrRed, true); }
         FireAlert("SELL");
      }
