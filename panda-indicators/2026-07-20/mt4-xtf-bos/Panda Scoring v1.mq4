//+------------------------------------------------------------------+
//|  Panda Scoring v1                                                 |
//|  Clean scoring panel built on the v5 engine. Shows only: BIAS, GAP,  |
//|  EXECUTION, SETUP, MARKET STRUCTURE H1/H4, and PANDA LINES (price    |
//|  above / inbetween / below). SuperTrend + BB lines off by default.   |
...
input int          Panel_Width  = 340;                 // panel width (px)
...
input bool         ShowScoreGrid   = false;            // optional per-pair scores grid
...
int OnInit()
{
   ...
   IndicatorShortName("Panda Scoring v1");
...
color StatusColor(const string v)
{
   ...
   if(v == "PRICE ABOVE") return(Panel_BuyColor);
   if(v == "PRICE BELOW") return(Panel_SellColor);
   if(v == "PRICE INBETWEEN") return(Panel_WaitColor);
...
void PanelRow(const int r, const string label, const string value, const color clr)
{
   ...
   PanelLabel(PANEL + "v" + IntegerToString(r), _pLeft + 150, y, value, clr);
}
...
void DrawPanel()
{
   ...
   int N = 8;
   if(ShowScoreGrid) N += 8;   // optional SCORES + ADV per-pair grids
   ...
   string plText  = (PLZone == "ABOVE") ? "PRICE ABOVE" : (PLZone == "BELOW") ? "PRICE BELOW" : (PLZone == "BETWEEN") ? "PRICE INBETWEEN" : "--";
   int r = 0;
   PanelRow(r++, "PANDA SCORING", CurrentPair == "" ? Symbol() : CurrentPair, Panel_TitleColor);
   ...
   PanelRow(r++, "SETUP",      SignalStatus, StatusColor(SignalStatus));
   PanelRow(r++, "MARKET STRUCTURE H1", BoxH1Trend, StatusColor(BoxH1Trend));
   PanelRow(r++, "MARKET STRUCTURE H4", BoxH4Trend, StatusColor(BoxH4Trend));
   PanelRow(r++, "PANDA LINES", plText,      StatusColor(plText));
