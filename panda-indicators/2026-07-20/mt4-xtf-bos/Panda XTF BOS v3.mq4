//+------------------------------------------------------------------+
//|  Panda XTF BOS v3                                                 |
//|  v3: score = ABSOLUTE EXTREME (dominant-side total, e.g. +3), not   |
//|  net. Adds ADV (advance) scoring: same box vote with boxes shifted  |
//|  one separator right, shown as a second grid. Cells show +pos/-neg. |
...
int OnInit()
{
   ...
   IndicatorShortName("Panda XTF BOS v3");
...
void FireAlert(const string dir)
{
   ...
   string msg = "Panda XTF BOS v2: " + dir + " TRIGGER " + CurrentPair + " " + Symbol();
...
void ProcessClosedShift(const int b, const int rates_total, const bool isLive)
{
   ...
   if(bosBull && ShowBOS) DrawArrowMark("bos", Time[b], Low[b],  233, clrLime, false, 1);
   if(bosBear && ShowBOS) DrawArrowMark("bos", Time[b], High[b], 234, clrRed,  true,  1);
   ...
   if(isLive)
   {
      ...
      if(buyReady && bosBull)
      {
         ...
         if(ShowTriggers) { DrawArrowMark("sig", Time[b], Low[b], 233, clrLime, false, 3); DrawSigText("sig", Time[b], Low[b], "BUY", clrLime, false); }
         FireAlert("BUY");
      }
      else if(sellReady && bosBear)
      {
         SignalStatus = "SELL TRIGGER";
         if(ShowTriggers) { DrawArrowMark("sig", Time[b], High[b], 234, clrRed, true, 3); DrawSigText("sig", Time[b], High[b], "SELL", clrRed, true); }
         FireAlert("SELL");
      }
...
// Backfill BOS + flip arrows across history, then only new closed bars after.
void ProcessBars(const int rates_total)
{
   int startB = rates_total - 2 - SwingLength;   // furthest back with a confirmable pivot
   if(startB < 1) startB = 1;
   for(int b = startB; b >= 1; b--)
   {
      if(Time[b] <= _lastProcTime) continue;
      ProcessClosedShift(b, rates_total, (b == 1));
   }
...
void DrawPanel()
{
   ...
   int N = 13;
   if(ShowScoreGrid) N += 18;  // main: spacer+header+7 ; ADV: spacer+header+7
   ...
   if(ShowScoreGrid)
   {
      ...
      for(int ci = 0; ci < 7; ci++)
      {
         ...
         PanelLabel(PANEL + "gd" + IntegerToString(ci), _pLeft + 118, gy, CellText(PosD1[ci], NegD1[ci]), CellColor(ScoresD1[ci]));
         PanelLabel(PANEL + "ge" + IntegerToString(ci), _pLeft + 186, gy, CellText(PosH4[ci], NegH4[ci]), CellColor(ScoresH4[ci]));
         PanelLabel(PANEL + "gf" + IntegerToString(ci), _pLeft + 254, gy, CellText(PosH1[ci], NegH1[ci]), CellColor(ScoresH1[ci]));
      }
      r++;  // spacer row
      ...
      {
         int ay = _pTop + 6 + r * _rowH; r++;
         PanelLabel(PANEL + "ac" + IntegerToString(ai), _pLeft + 8,   ay, Currencies[ai], Panel_TextColor);
         PanelLabel(PANEL + "ad" + IntegerToString(ai), _pLeft + 118, ay, CellText(AdvPosD1[ai], AdvNegD1[ai]), CellColor(AdvD1[ai]));
         PanelLabel(PANEL + "ae" + IntegerToString(ai), _pLeft + 186, ay, CellText(AdvPosH4[ai], AdvNegH4[ai]), CellColor(AdvH4[ai]));
         PanelLabel(PANEL + "af" + IntegerToString(ai), _pLeft + 254, ay, CellText(AdvPosH1[ai], AdvNegH1[ai]), CellColor(AdvH1[ai]));
      }
   }
}
