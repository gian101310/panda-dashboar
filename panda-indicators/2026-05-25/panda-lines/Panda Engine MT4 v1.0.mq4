//+------------------------------------------------------------------+
//| Panda Engine MT4 v1                                              |
//| Combined: Scoring v1.1 + Panda Lines v4.1                       |
...
input int    BoxCalcTF        = PERIOD_H1;
input int    WagBox3Days      = 2;
input int    WagBox3Offset    = 1;
input int    WagBox1Weeks     = 2;
input int    WagBox1Offset    = 1;
input int    WagBox2Months    = 2;
input int    WagBox2Offset    = 1;
...
input double SR_ZoneWidth     = 0.01;
input int    SR_ExtendLeft    = 3000;
input int    SR_ExtendRight   = 3000;
...
input color  WagBox2Color     = C'70,120,255';
input color  WagBox1Color     = C'0,200,120';
input color  WagBox3Color     = C'255,160,50';
input int    BoxLineWidth     = 1;
...
input bool   TestMode              = true;     // USE pe_ PREFIX — safe for side-by-side testing
input bool   ExportForEngine       = true;     // write mt4/tbg files (engine reads these)
input bool   ExportPandaScore      = true;     // write panda_score file
...
// Panel state
string PanelPrefix   = "PandaEng_";
string BoxPrefix     = "PandaEngBox_";
string STPrefix      = "PandaEngST_";
...
int OnInit()
{
   IndicatorShortName("Panda Engine v1");
                ...
                const int &spread[])
{
   ...
   // Refresh scoring on calc if timer interval elapsed
                         ...
                         const double &close[])
{
   ...
   double prev_up = IsValidVal(UpBand[i + 1]) ? UpBand[i + 1] : cur_up;
   double prev_dn = IsValidVal(DnBand[i + 1]) ? DnBand[i + 1] : cur_dn;

   if(close[i + 1] > prev_up) cur_up = MathMax(cur_up, prev_up);
   if(close[i + 1] < prev_dn) cur_dn = MathMin(cur_dn, prev_dn);

   UpBand[i] = cur_up;
   DnBand[i] = cur_dn;

   double prev_trend = IsValidVal(TrendDir[i + 1]) ? TrendDir[i + 1] : 1.0;
   ...
   if(ST_ShowSignals)
   {
      ...
      if(TrendDir[i] == 1.0 && TrendDir[i + 1] == -1.0)
         DrawArrow(STPrefix + "buy_" + IntegerToString((int)time[i]), time[i], st_value - offset, Lime, 233);
      if(TrendDir[i] == -1.0 && TrendDir[i + 1] == 1.0)
         DrawArrow(STPrefix + "sell_" + IntegerToString((int)time[i]), time[i], st_value + offset, Red, 234);
   }
}

// --- BB Trend Line Calculation ---
                      ...
                      const double &close[])
{
   ...
   double prev_tl = (IsValidVal(BBTrendLine[i + 1]) && BBTrendLine[i + 1] != 0.0) ? BBTrendLine[i + 1] : close[i];
   ...
   int bb_signal = 0;
   if(prev_close > prev_upper) bb_signal = 1;
   else if(prev_close < prev_lower) bb_signal = -1;
   ...
   double prev_itrend = IsValidVal(BBITrend[i + 1]) ? BBITrend[i + 1] : 0.0;
   BBITrend[i] = prev_itrend;
   if(cur_tl > prev_tl) BBITrend[i] = 1.0;
   else if(cur_tl < prev_tl) BBITrend[i] = -1.0;
   ...
   if(!BB_HideLabels)
   {
      ...
      if(BBITrend[i] == 1.0 && BBITrend[i + 1] == -1.0)
         DrawArrow(STPrefix + "bbBuy_" + IntegerToString((int)time[i]), time[i], cur_tl - label_offset, DodgerBlue, 117);
      if(BBITrend[i] == -1.0 && BBITrend[i + 1] == 1.0)
         DrawArrow(STPrefix + "bbSell_" + IntegerToString((int)time[i]), time[i], cur_tl + label_offset, Red, 117);
   }
}
...
{
   RemoveObjectsByPrefix(STPrefix + "SR_");

   datetime time_left = time[MathMin(rates_total - 1, SR_ExtendLeft)];
   datetime time_right = time[0] + PeriodSeconds() * SR_ExtendRight;
...
void ComputePLZone()
{
   PLSTValue = GetCurrentST();
   PLFLValue = GetCurrentFL();
   PLZone = "--";
   PLBias = "--";
   PLG1Valid = false;

   if(PLSTValue <= 0 || PLFLValue <= 0) return;
   ...
   double upper = MathMax(PLSTValue, PLFLValue);
   double lower = MathMin(PLSTValue, PLFLValue);

   if(price > upper)      PLZone = "ABOVE";
   else if(price < lower) PLZone = "BELOW";
   else                   PLZone = "BETWEEN";

   // PL Bias from trend direction
   bool stBull = (ArraySize(STBullish) > 0 && STBullish[0] != EMPTY_VALUE && STBullish[0] > 0);
   bool flBull = (ArraySize(BBTrendBull) > 0 && BBTrendBull[0] != EMPTY_VALUE && BBTrendBull[0] > 0);
   bool stBear = (ArraySize(STBearish) > 0 && STBearish[0] != EMPTY_VALUE && STBearish[0] > 0);
   bool flBear = (ArraySize(BBTrendBear) > 0 && BBTrendBear[0] != EMPTY_VALUE && BBTrendBear[0] > 0);

   if(stBull && flBull)      PLBias = "BUY";
   else if(stBear && flBear) PLBias = "SELL";
   else                      PLBias = "MIXED";

   // G1: Zone confirms gap direction (exact match to engine confluence logic)
   if(MainGap >= GapThreshold && PLZone == "ABOVE")
      PLG1Valid = true;
   else if(MainGap <= -GapThreshold && PLZone == "BELOW")
      PLG1Valid = true;
   else
      PLG1Valid = false;
}
...
void ExportAllFiles()
{
   ...
   // panda_score_*.txt: our panel uses internal data (not this file),
   // but we write it so any other tool/panel can read it too.
   // Engine also writes this file — harmless, last writer wins.
   if(ExportPandaScore) WritePandaScoreFile();

   // mt4_*.txt + tbg_*.txt: these REPLACE what the original closed-source
   // indicator and TBG used to write. The engine reads these files.
   if(ExportForEngine)
   {
      WriteMt4File();
      WriteTbgFile();
...
void DrawPanel()
{
   ...
   string titleText = TestMode ? "PANDA ENGINE v1 [TEST]" : "PANDA ENGINE v1";
...
void HidePanel()
{
   ...
   CreateLabel(PanelPrefix + "mini_label", x + 8, y + 5, "PANDA ENG v1", C'0,180,255', 9, "Arial Bold");
