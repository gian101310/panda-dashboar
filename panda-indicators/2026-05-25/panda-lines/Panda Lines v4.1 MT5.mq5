...
input int      Panel_X         = 20;    // default left offset
input int      Panel_Y         = 30;    // default bottom offset
...
string BoxPrefix = "PandaBoxV41_";
int Panel_Height = 376;
...
string EngineExecution  = "--";
datetime LastEngineRead = 0;
...
int DefaultPanelY()
{
   ...
   int y = chartH - Panel_Height - Panel_Y;
   return(y > 0 ? y : 0);
}
...
int OnInit()
{
   IndicatorSetString(INDICATOR_SHORTNAME, "Panda Lines v4.1 MT5");
...
void ReadEngineScoreFile(const string clean)
{
   ...
   while(!FileIsEnding(handle))
   {
      ...
      string key = TrimCopy(StringSubstr(line, 0, sep));
      string val = TrimCopy(StringSubstr(line, sep + 1));

      if(key == "GAP")                  EngineGap        = (int)StringToInteger(val);
      else if(key == "BIAS")            EngineBias       = val;
      else if(key == "CONFIDENCE")      EngineConfidence = val;
      else if(key == "EXECUTION")       EngineExecution  = val;
...
void DrawPanel()
{
   ...
   CreateLbl(PanelPrefix + "title", x + 8, y + 5, "PANDA LINES v4.1", C'0,180,255', 10, "Arial Bold");
   ...
   CreatePanelLine(line++, x, cy, "GAP SCORE   " + SignedInt(ScoreGap), biasClr, Panel_FontSize + 1, "Arial Bold");
   CreateRect(PanelPrefix + "sep1", x + 8, cy + line * lineH + 3, w - 16, 1, C'50,50,70');
   line++;

   string g1 = ScorePLG1 == 1 ? "OK" : "WAIT";
   CreatePanelLine(line++, x, cy, "CONFLUENCE  " + IntegerToString(ScoreConfluence), ConfluenceColor(), Panel_FontSize, "Arial Bold");
   CreatePanelLine(line++, x, cy, "PL ZONE  " + ScorePLZone + "   G1  " + g1, PLColor(), Panel_FontSize, "Arial");
   CreatePanelLine(line++, x, cy, "BOX  H1 " + ShortBox(ScoreBoxH1) + "   H4 " + ShortBox(ScoreBoxH4), Panel_TextColor, Panel_FontSize, "Arial");
   CreatePanelLine(line++, x, cy, "MOMENTUM  " + ScoreMomentum + "   STRENGTH  " + DoubleToString(ScoreStrength, 1), MomentumColor(), Panel_FontSize, "Arial");
   CreateRect(PanelPrefix + "sep2", x + 8, cy + line * lineH + 3, w - 16, 1, C'50,50,70');
   line++;

   CreatePanelLine(line++, x, cy, "MT4 SOURCE", C'120,120,150', 8, "Arial Bold");
   CreatePanelLine(line++, x, cy, ClipText(PanelBaseLine, 46), Panel_TextColor, Panel_FontSize - 1, "Arial");
   CreatePanelLine(line++, x, cy, ClipText(PanelQuoteLine, 46), Panel_TextColor, Panel_FontSize - 1, "Arial");
   CreatePanelLine(line++, x, cy, ClipText(PanelAdvBase, 46), C'160,160,180', Panel_FontSize - 1, "Arial");
   CreatePanelLine(line++, x, cy, ClipText(PanelAdvQuote, 46), C'160,160,180', Panel_FontSize - 1, "Arial");
   CreatePanelLine(line++, x, cy, ClipText(PanelAtrLine, 46), C'160,160,180', Panel_FontSize - 1, "Arial");
   CreatePanelLine(line++, x, cy, ClipText(PanelSpreadLine, 46), C'160,160,180', Panel_FontSize - 1, "Arial");

   if(LastFileRead > 0)
   {
      int age = (int)(TimeCurrent() - LastFileRead);
      string freshStr = (age < 120) ? "MT4 LIVE" : "MT4 " + IntegerToString(age/60) + "m";
      color freshClr = (age < 120) ? C'0,180,255' : C'180,100,100';
      if(LastEngineRead > 0)
      {
         int engAge = (int)(TimeCurrent() - LastEngineRead);
         freshStr += (engAge < 120) ? " / ENG LIVE" : " / ENG " + IntegerToString(engAge/60) + "m";
      }
...
string ClipText(string text, const int maxLen)
{
   if(text == "")
      return("--");
...
string ShortBox(const string value)
{
...
{
   if(ScoreConfluence >= 90) return(Panel_BuyColor);
   if(ScoreConfluence >= 75) return(C'0,180,255');
   if(ScoreConfluence >= 60) return(C'255,215,102');
   if(ScoreConfluence >= 40) return(C'255,170,68');
   return(Panel_WaitColor);
}

color PLColor()
{
   if(ScorePLZone == "ABOVE") return(Panel_BuyColor);
   if(ScorePLZone == "BELOW") return(Panel_SellColor);
   if(ScorePLZone == "BETWEEN") return(C'255,170,68');
   return(Panel_WaitColor);
}

color MomentumColor()
{
   if(ScoreMomentum == "STRONG" || ScoreMomentum == "BUILDING") return(Panel_BuyColor);
   if(ScoreMomentum == "SPARK") return(C'255,215,102');
   if(ScoreMomentum == "COOLING" || ScoreMomentum == "FADING") return(C'255,170,68');
   if(ScoreMomentum == "REVERSING") return(Panel_SellColor);
   return(Panel_WaitColor);
}
