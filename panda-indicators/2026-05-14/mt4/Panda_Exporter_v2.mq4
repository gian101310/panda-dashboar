...
input int    ExportIntervalSeconds = 5;      // Export interval (seconds)
...
//+------------------------------------------------------------------+
//| Strip broker suffix to get clean pair name for app.py             |
//+------------------------------------------------------------------+
string CleanSymbol(string brokerSym)
{
   int total = ArraySize(PAIRS);
   for(int i = 0; i < total; i++)
   {
      if(StringFind(brokerSym, PAIRS[i]) == 0)
         return PAIRS[i];
   }
   // Try uppercase match
   string upper = brokerSym;
   StringToUpper(upper);
   for(int i = 0; i < total; i++)
   {
   ...
   return brokerSym; // fallback — use as-is
}
...
void ExportBox(long chartId, int handle, string name)
{
   ...
   int digits  = (int)MarketInfo(ChartSymbol(chartId), MODE_DIGITS);
   FileWrite(handle,
      "BOX|" + name + "|" +
...
void ExportChart(long chartId, string symbol)
{
   ...
   string fileName = filePrefix + clean + ".txt";
   int handle = -1;
   for(int attempt = 0; attempt < 5; attempt++)
   {
      handle = FileOpen(fileName, FILE_WRITE | FILE_TXT | FILE_COMMON);
      if(handle != INVALID_HANDLE) break;
      Sleep(200 + attempt * 200);  // 200, 400, 600, 800, 1000ms
   }
   if(handle == INVALID_HANDLE) return;  // silent skip — next cycle will catch it
