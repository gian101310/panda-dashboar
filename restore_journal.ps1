...
function pipsColor(v) { return v > 0 ? '#00ff9f' : v < 0 ? '#ff4d6d' : '#445566'; }
function plColor(v) { return v > 0 ? '#00ff9f' : v < 0 ? '#ff4d6d' : '#445566'; }
...
function AddTradeModal({ strategies, onClose, onAdded }) {
  ...
  const inp = {background:'#06080f',border:'1px solid #1a2540',borderRadius:5,padding:'7px 10px',color:'#e8eaf0',fontFamily:raj,fontSize:14,width:'100%',boxSizing:'border-box'};
  const lbl = {fontFamily:mono,fontSize:9,letterSpacing:2,color:'#445566',display:'block',marginBottom:4};
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{background:'#0a0e1a',border:'1px solid #1e3060',borderRadius:12,padding:28,width:500,maxHeight:'90vh',overflowY:'auto',display:'flex',flexDirection:'column',gap:14}}>
  ...
  async function handleFile(e) {
    ...
    try {
      ...
      if (ext === 'xlsx' || ext === 'xls' || ext === 'xlsm') {
        ...
        throw new Error('Unsupported file type. Use XLSX, HTML, or CSV.');
      }

      if (rows.length === 0) throw new Error('No data rows found in file');
      setStatus(``Found `${rows.length} rows, uploading...``);

      // Split into chunks of 100 rows and upload sequentially
      ...
      for (let chunk = 0; chunk < rows.length; chunk += CHUNK_SIZE) {
        ...
        setStatus(``Uploading chunk `${chunkNum}/`${totalChunks} (`${totalImported} imported so far)...``);

        const res = await fetch('/api/journal-upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ rows: batch, filename: file.name }),
        });
        const d = await res.json();
  ...
  return (
    <div style={{background:'rgba(0,180,255,0.05)',border:'1px solid rgba(0,180,255,0.2)',borderRadius:8,padding:'14px 16px',display:'flex',flexDirection:'column',gap:8}}>
      ...
      <div style={{display:'flex',gap:20,flexWrap:'wrap'}}>
        <div>
          <div style={{fontFamily:mono,fontSize:9,color:'#00ff9f',marginBottom:3}}>XLSX (Recommended)</div>
          <div style={{fontFamily:mono,fontSize:8,color:'#445566'}}>History: right-click Export - Excel format</div>
        </div>
        <div>
          <div style={{fontFamily:mono,fontSize:9,color:'#ffd166',marginBottom:3}}>HTML</div>
          <div style={{fontFamily:mono,fontSize:8,color:'#445566'}}>History: right-click Export - HTML format</div>
        </div>
      </div>
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
        <input ref={fileRef} type="file" accept=".xlsx,.xls,.html,.htm,.csv" onChange={handleFile} style={{display:'none'}}/>
        <button onClick={()=>fileRef.current?.click()} disabled={loading} style={{background:'rgba(0,180,255,0.1)',border:'1px solid #00b4ff',borderRadius:5,color:'#00b4ff',fontFamily:mono,fontSize:9,letterSpacing:2,padding:'7px 16px',cursor:'pointer'}}>
          {loading ? 'IMPORTING...' : 'CHOOSE FILE (XLSX / HTML)'}
        </button>
...
function StrategyManager({ strategies, onRefresh }) {
  ...
  async function save() {
    setLoading(true);
    if (editId) {
      await fetch('/api/strategies', { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ id:editId, ...form }) });
    } else {
      await fetch('/api/strategies', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(form) });
    }
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
      ...
      {(showAdd || editId) && (
        <div style={{background:'#060810',border:'1px solid #1a2540',borderRadius:8,padding:'14px 16px',display:'flex',flexDirection:'column',gap:10}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            ...
            <div><label style={lbl}>COLOR</label>
              <div style={{display:'flex',gap:5}}>
                {STRATEGY_COLORS.map(c=><div key={c} onClick={()=>setForm(f=>({...f,color:c}))} style={{width:22,height:22,borderRadius:'50%',background:c,cursor:'pointer',border:form.color===c?``2px solid #fff``:'2px solid transparent'}}/>)}
              </div>
            </div>
          </div>
          ...
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>{setShowAdd(false);setEditId(null);}} style={{flex:1,background:'transparent',border:'1px solid #1a2540',borderRadius:5,color:'#445566',fontFamily:mono,fontSize:10,padding:'7px',cursor:'pointer'}}>CANCEL</button>
      ...
      {strategies.length === 0 && !showAdd && (
        <div style={{textAlign:'center',padding:30,fontFamily:mono,fontSize:10,color:'#2a3550',letterSpacing:2}}>NO STRATEGIES YET - Click + NEW STRATEGY</div>
      )}

      <div style={{display:'flex',flexDirection:'column',gap:6}}>
        {strategies.map(s=>(
          <div key={s.id} style={{background:'#060810',border:``1px solid `${s.color}33``,borderRadius:8,padding:'10px 14px',display:'flex',alignItems:'flex-start',gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              {s.description&&<div style={{fontFamily:raj,fontSize:13,color:'#6677aa',marginTop:2}}>{s.description}</div>}
              {s.rules&&<pre style={{fontFamily:mono,fontSize:9,color:'#445566',marginTop:6,whiteSpace:'pre-wrap',letterSpacing:0.5}}>{s.rules}</pre>}
            </div>
            <div style={{display:'flex',gap:6,flexShrink:0}}>
              <button onClick={()=>{setEditId(s.id);setShowAdd(false);setForm({name:s.name,description:s.description,rules:s.rules,color:s.color});}} style={{background:'rgba(0,180,255,0.08)',border:'1px solid #1e306066',borderRadius:4,color:'#00b4ff',fontFamily:mono,fontSize:8,padding:'4px 8px',cursor:'pointer'}}>EDIT</button>
...
function JournalStats({ trades }) {
  ...
  const pairStats = {};
  closed.forEach(t => {
    if (!pairStats[t.symbol]) pairStats[t.symbol] = { pl:0, pips:0, count:0, wins:0 };
    pairStats[t.symbol].pl += t.profit_loss || 0;
    pairStats[t.symbol].pips += t.profit_loss_pips || 0;
    pairStats[t.symbol].count++;
    if ((t.profit_loss || 0) > 0) pairStats[t.symbol].wins++;
  });
  const pairList = Object.entries(pairStats).sort((a,b) => b[1].pl - a[1].pl);

  // Momentum accuracy
  const momStats = {};
  closed.filter(t => t.momentum_at_entry).forEach(t => {
    const m = t.momentum_at_entry;
    if (!momStats[m]) momStats[m] = { count:0, wins:0 };
    momStats[m].count++;
    if ((t.profit_loss || 0) > 0) momStats[m].wins++;
  });

  const card = (label, value, color, sub) => (
    <div style={{background:'#0a0e1a',border:'1px solid #1a2540',borderRadius:8,padding:'10px 14px',flex:1,minWidth:110}}>
      <div style={{fontFamily:mono,fontSize:8,letterSpacing:2,color:'#445566',marginBottom:4}}>{label}</div>
      <div style={{fontFamily:orb,fontSize:18,fontWeight:700,color,textShadow:``0 0 10px `${color}66``}}>{value}</div>
      {sub&&<div style={{fontFamily:mono,fontSize:8,color:'#2a3550',marginTop:2}}>{sub}</div>}
    </div>
  );
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        {/* Best/Worst */}
        <div style={{background:'#0a0e1a',border:'1px solid #1a2540',borderRadius:8,padding:'12px 14px'}}>
          <div style={{fontFamily:orb,fontSize:10,fontWeight:700,color:'#ffd166',letterSpacing:3,marginBottom:8}}>BEST / WORST TRADES</div>
          {bestTrade&&<div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid #111827'}}>
            <span style={{fontFamily:mono,fontSize:9,color:'#00ff9f'}}>* {bestTrade.symbol} {bestTrade.direction}</span>
            <span style={{fontFamily:mono,fontSize:9,color:'#00ff9f'}}>+`${(bestTrade.profit_loss||0).toFixed(2)}</span>
          </div>}
          {worstTrade&&<div style={{display:'flex',justifyContent:'space-between',padding:'5px 0'}}>
            <span style={{fontFamily:mono,fontSize:9,color:'#ff4d6d'}}>x {worstTrade.symbol} {worstTrade.direction}</span>
            <span style={{fontFamily:mono,fontSize:9,color:'#ff4d6d'}}>`${(worstTrade.profit_loss||0).toFixed(2)}</span>
          </div>}
        </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:4,maxHeight:120,overflowY:'auto'}}>
            {pairList.slice(0,6).map(([sym,s])=>(
              <div key={sym} style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <span style={{fontFamily:mono,fontSize:9,color:'#8892aa'}}>{sym}</span>
                <div style={{display:'flex',gap:8}}>
                  <span style={{fontFamily:mono,fontSize:9,color:'#445566'}}>{s.count}T</span>
      ...
      {Object.keys(momStats).length > 0 && (
        <div style={{background:'#0a0e1a',border:'1px solid #1a2540',borderRadius:8,padding:'12px 14px'}}>
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            {Object.entries(momStats).map(([m,s])=>{
              const wr = ((s.wins/s.count)*100).toFixed(0);
              return (
                <div key={m} style={{background:'#060810',border:'1px solid #1a2540',borderRadius:6,padding:'7px 12px',minWidth:100}}>
                  <div style={{fontFamily:mono,fontSize:9,color:'#445566'}}>{m}</div>
                  <div style={{fontFamily:orb,fontSize:16,fontWeight:700,color:parseInt(wr)>=55?'#00ff9f':parseInt(wr)>=45?'#ffd166':'#ff4d6d'}}>{wr}%</div>
                  <div style={{fontFamily:mono,fontSize:8,color:'#2a3550'}}>{s.count} trades</div>
                </div>
              );
            })}
          </div>
...
export default function JournalPage() {
  ...
  useEffect(() => {
    fetch('/api/me').then(r=>r.json()).then(d => {
      ...
      if (!d.feature_access?.includes('journal') && d.role !== 'admin' && d.role !== 'vip') {
  ...
  function toggleCol(col) {
    setHiddenCols(h => h.includes(col) ? h.filter(c=>c!==col) : [...h, col]);
  }
  ...
  async function clearAllTrades() {
    ...
    const res = await fetch('/api/journal', { method:'DELETE', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ clearAll: true }) });
    const d = await res.json();
    if (d.ok) { alert('All trades deleted!'); loadTrades(); }
  ...
  function downloadCSV() {
    ...
    sorted.forEach(t => {
      const vals = visibleCols.map(c => {
        ...
        if (!key) return '';
        const v = t[key];
        if (c === 'ENTRY TIME' || c === 'EXIT TIME') return v ? new Date(v).toLocaleString() : '';
        if (c === 'DURATION') { if (!t.entry_time || !t.exit_time) return ''; const mins = Math.round((new Date(t.exit_time)-new Date(t.entry_time))/60000); return ```${mins}m``; }
    ...
    a.download = ``panda_journal_`${new Date().toISOString().slice(0,10)}.csv``;
    a.click();
  }

  const hdr = {padding:'8px 10px',fontFamily:mono,fontSize:9,letterSpacing:2,color:'#445566',textAlign:'left',borderBottom:'1px solid #1a2540',fontWeight:400,cursor:'pointer',whiteSpace:'nowrap'};
  const tdc = {padding:'8px 10px',fontFamily:raj,fontSize:13,color:'#8892aa',verticalAlign:'middle',whiteSpace:'nowrap'};

  return (
    <>
      <Head><title>PANDA - TRADE JOURNAL</title>
      ...
      <div style={{minHeight:'100vh',background:'#050810',display:'flex',flexDirection:'column'}}>
        <div style={{position:'fixed',inset:0,backgroundImage:'linear-gradient(rgba(0,180,255,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(0,180,255,0.025) 1px,transparent 1px)',backgroundSize:'40px 40px',pointerEvents:'none',zIndex:0}}/>

        {/* HEADER */}
        <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 20px',background:'#080c18',borderBottom:'1px solid #1a2540',position:'sticky',top:0,zIndex:100}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <span style={{fontSize:22}}>PANDA</span>
            <div><div style={{fontFamily:orb,fontWeight:900,fontSize:13,letterSpacing:4,color:'#ffd166'}}>TRADE JOURNAL</div><div style={{fontFamily:mono,fontSize:8,letterSpacing:3,color:'#2a3550'}}>{user?.username} . {user?.role?.toUpperCase()}</div></div>
          </div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,padding:'10px 20px',zIndex:1}}>
          <div style={{display:'flex',background:'#080c18',border:'1px solid #1a2540',borderRadius:7,overflow:'hidden'}}>
            {['JOURNAL','STATS','GAMEPLAY'].map((t,i)=>(
              <button key={t} onClick={()=>setTab(t)} style={{background:tab===t?'rgba(255,209,102,0.15)':'transparent',border:'none',borderRight:i<2?'1px solid #1a2540':'none',color:tab===t?'#ffd166':'#445566',fontFamily:mono,fontSize:9,letterSpacing:2,padding:'7px 16px',cursor:'pointer'}}>{t}</button>
            ))}
          </div>
          {tab==='JOURNAL'&&<>
        ...
        <div style={{flex:1,padding:'0 20px 20px',zIndex:1,display:'flex',flexDirection:'column',gap:12}}>
          ...
          {tab==='JOURNAL'&&<>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
              <select style={{background:'#080c18',border:'1px solid #1a2540',borderRadius:5,padding:'6px 10px',color:'#8892aa',fontFamily:mono,fontSize:9,cursor:'pointer'}} value={filterSymbol} onChange={e=>setFilterSymbol(e.target.value)}>
                <option value="">ALL PAIRS</option>{ALL_PAIRS.map(p=><option key={p} value={p}>{p}</option>)}
              </select>
              <select style={{background:'#080c18',border:'1px solid #1a2540',borderRadius:5,padding:'6px 10px',color:'#8892aa',fontFamily:mono,fontSize:9,cursor:'pointer'}} value={filterDir} onChange={e=>setFilterDir(e.target.value)}>
                <option value="">ALL DIRECTIONS</option><option value="BUY">BUY</option><option value="SELL">SELL</option>
              </select>
              <select style={{background:'#080c18',border:'1px solid #1a2540',borderRadius:5,padding:'6px 10px',color:'#8892aa',fontFamily:mono,fontSize:9,cursor:'pointer'}} value={filterStatus} onChange={e=>setFilterStatus(e.target.value)}>
                <option value="">ALL STATUS</option><option value="OPEN">OPEN</option><option value="CLOSED">CLOSED</option>
              </select>
              <span style={{fontFamily:mono,fontSize:9,color:'#2a3550'}}>{sorted.length} trades</span>

              {/* Column toggle */}
              <div style={{marginLeft:'auto',display:'flex',gap:4,flexWrap:'wrap'}}>
                {['SL','TP','COMMISSION','SWAP','SOURCE'].map(col=>(
                  <button key={col} onClick={()=>toggleCol(col)} style={{background:hiddenCols.includes(col)?'transparent':'rgba(0,180,255,0.08)',border:``1px solid `${hiddenCols.includes(col)?'#1a2540':'#00b4ff44'}``,borderRadius:4,color:hiddenCols.includes(col)?'#2a3550':'#00b4ff',fontFamily:mono,fontSize:7,padding:'3px 7px',cursor:'pointer'}}>{col}</button>
                ))}
              </div>
            </div>

            {/* Table */}
            {loading ? <div style={{textAlign:'center',padding:60,fontFamily:mono,fontSize:11,color:'#445566',letterSpacing:3}}>LOADING...</div> : (
              <div style={{overflowX:'auto',borderRadius:10,border:'1px solid #1a2540'}}>
                <table style={{width:'100%',borderCollapse:'collapse',background:'#080c18'}}>
                  <thead>
                  ...
                  <tbody>
                    {sorted.length===0
                      ?<tr><td colSpan={ALL_COLS.length} style={{textAlign:'center',padding:40,fontFamily:mono,fontSize:10,color:'#2a3550'}}>NO TRADES YET - Log a trade or import CSV</td></tr>
                      :sorted.map((t,idx)=>{
                        const dur = t.entry_time&&t.exit_time ? Math.round((new Date(t.exit_time)-new Date(t.entry_time))/60000) : null;
                        const stratColor = strategies.find(s=>s.name===t.strategy_name)?.color || '#445566';
                        const colData = {
                          '#': <span style={{fontFamily:mono,fontSize:9,color:'#2a3550'}}>{idx+1}</span>,
                          'PAIR': <span style={{fontFamily:orb,fontSize:11,fontWeight:700,color:'#e8eaf0'}}>{t.symbol}</span>,
                          'DIR': <span style={{fontFamily:mono,fontSize:10,color:t.direction==='BUY'?'#00ff9f':'#ff4d6d',fontWeight:700}}>{t.direction}</span>,
                          'VOLUME': <span style={{fontFamily:mono,fontSize:10,color:'#8892aa'}}>{t.volume}</span>,
                          'ENTRY': <span style={{fontFamily:mono,fontSize:10,color:'#8892aa'}}>{t.entry_price||'-'}</span>,
                          'EXIT': <span style={{fontFamily:mono,fontSize:10,color:'#8892aa'}}>{t.exit_price||'-'}</span>,
                          ...
                          'ENTRY TIME': <span style={{fontFamily:mono,fontSize:9,color:'#445566'}}>{formatDt(t.entry_time)}</span>,
                          'EXIT TIME': <span style={{fontFamily:mono,fontSize:9,color:'#445566'}}>{formatDt(t.exit_time)}</span>,
                          'DURATION': <span style={{fontFamily:mono,fontSize:9,color:'#2a3550'}}>{dur?```${dur}m``:'-'}</span>,
                          'GAP@ENTRY': <span style={{fontFamily:mono,fontSize:10,color:t.gap_at_entry>=5?'#00ff9f':t.gap_at_entry<=-5?'#ff4d6d':'#445566'}}>{t.gap_at_entry!==null?```${t.gap_at_entry>0?'+':''}`${t.gap_at_entry}``:'-'}</span>,
                          'MOMENTUM': <span style={{fontFamily:mono,fontSize:9,color:'#445566'}}>{t.momentum_at_entry||'-'}</span>,
                          'STRATEGY': t.strategy_name?<span style={{fontFamily:mono,fontSize:9,color:stratColor,background:stratColor+'18',border:``1px solid `${stratColor}33``,borderRadius:3,padding:'1px 5px'}}>{t.strategy_name}</span>:<span style={{color:'#2a3550'}}>-</span>,
                          'SOURCE': <span style={{fontFamily:mono,fontSize:8,color:t.source==='ctrader'?'#00b4ff':'#445566'}}>{t.source==='ctrader'?'cTrader':'manual'}</span>,
                          'NOTES': <span style={{fontFamily:raj,fontSize:12,color:'#445566',maxWidth:150,overflow:'hidden',textOverflow:'ellipsis',display:'block'}}>{t.notes||'-'}</span>,
                          'ACTION': t.source==='manual'?<button onClick={()=>deleteTrade(t.id)} style={{background:'rgba(255,77,109,0.08)',border:'1px solid #ff4d6d44',borderRadius:4,color:'#ff4d6d',fontFamily:mono,fontSize:8,padding:'3px 7px',cursor:'pointer'}}>DEL</button>:<span style={{color:'#2a3550',fontFamily:mono,fontSize:8}}>sync</span>,
                        };
                        return (
                          <tr key={t.id||idx} style={{borderBottom:'1px solid #111827',background:(t.profit_loss||0)>0?'rgba(0,255,159,0.01)':'rgba(255,77,109,0.01)'}}>
          ...
          {/* STATS TAB */}
          {tab==='STATS'&&(loading?<div style={{textAlign:'center',padding:60,fontFamily:mono,fontSize:11,color:'#445566'}}>LOADING...</div>:<JournalStats trades={trades}/>)}
...
'@
[System.IO.File]::WriteAllText("$PWD\pages\journal.js", $content, [System.Text.UTF8Encoding]::new($false))
Write-Host "Written: $((Get-Item pages\journal.js).Length) bytes"
git add pages\journal.js
git commit -m "Restore journal.js full file"
