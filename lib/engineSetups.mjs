...
function priceFromPips(symbol, pips) {
  return num(pips) * pipSize(symbol);
}
...
function supertrendLine(row) {
  for (const key of ['pl_st', 'supertrend_line', 'supertrend', 'supertrendLine']) {
    if (row?.[key] != null && Number.isFinite(Number(row[key]))) return num(row[key]);
  }
  return null;
}
...
export function classifyPandaLinesPbSetup(row) {
  if (!row || row.hard_invalid) return null;
  const gap = num(row.gap);
  if (Math.abs(gap) < 5) return null;
  const direction = row.bias === 'BUY' || row.bias === 'SELL' ? row.bias : directionFromGap(gap);
  if (direction === 'WAIT') return null;
  if (row.pl_g1_valid !== true || !plConfirmed(direction, row.pl_zone)) return null;
  return {
    symbol: row.symbol,
    direction,
    strategy: 'PLPB',
    gap,
    reason: 'gap >= 5 + Panda Lines confirmed',
  };
}

function setupForMode(row, now, mode) {
  return String(mode || '').toUpperCase() === 'PLPB'
    ? classifyPandaLinesPbSetup(row)
    : classifyEngineSetup(row, now);
}

export function buildPullbackPlan(row, currentPrice, now = new Date(), options = {}) {
  const setup = setupForMode(row, now, options.mode);
  ...
  if (setup.direction === 'BUY') {
    ...
    for (const e of entryLevels) {
      ...
      if (validTargets.length) {
        entry = e;
        takeProfit = ['INTRA', 'PLPB'].includes(setup.strategy) ? validTargets[validTargets.length - 1] : validTargets[0];
      }
      if (entry) break;
    }
  } else {
  ...
  if (setup.strategy === 'INTRA' || setup.strategy === 'PLPB') {
    const stLine = supertrendLine(row);
    if (stLine == null) return null;
    const stBufferPips = 10;
    const slPrice = setup.direction === 'BUY'
      ? round(stLine - priceFromPips(symbol, stBufferPips), dec)
      : round(stLine + priceFromPips(symbol, stBufferPips), dec);
    const slPips = toPips(symbol, entry.price - slPrice);
    if (slPips <= 0) return null;
    const tpPips = slPips * 2;
    const tpPrice = setup.direction === 'BUY'
      ? round(entry.price + priceFromPips(symbol, tpPips), dec)
      : round(entry.price - priceFromPips(symbol, tpPips), dec);
    const clearTargetPips = toPips(symbol, setup.direction === 'BUY' ? takeProfit.price - entry.price : entry.price - takeProfit.price);
    if (clearTargetPips < Math.max(50, tpPips)) return null;
    ...
    return {
      ...
      takeProfit: { label: '2R', price: tpPrice, pips: tpPips },
      stopLoss: { price: slPrice, pips: slPips, source: 'supertrend_line', bufferPips: stBufferPips },
      riskReward: 2,
      reason: `${setup.reason} + Supertrend SL`,
    };
  }
...
export function buildEngineChartObjects(setup, plan, nowIso = new Date().toISOString()) {
  ...
  const endIso = new Date(new Date(nowIso).getTime() + 6 * 60 * 60 * 1000).toISOString();
  return [
    {
    ...
    {
      object_type: 'horizontal_line',
      price1: plan.stopLoss.price,
      color: '#ff4d6d',
    },
    {
      object_type: 'horizontal_line',
      price1: plan.takeProfit.price,
      color: '#00ff9f',
    },
    {
      object_type: 'text',
