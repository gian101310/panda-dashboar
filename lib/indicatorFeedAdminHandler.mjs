import { hashOverlayToken, safeTokenEqual } from './ctraderOverlay.mjs';
...
export function createIndicatorFeedAdminHandler({
  ...
  return async function indicatorFeedAdminHandler(req, res) {
    ...
    if (req.method === 'PUT') {
      ...
      const persisted = await getSetting();
      if (!persisted?.token_hash || !safeTokenEqual(persisted.token_hash, tokenHash)) {
        return res.status(503).json({ error: 'Token activation could not be verified' });
      }
      return res.status(200).json({
        ok: true,
        configured: true,
