...
export function mergeStatsIntoGuardian(guardianState, statsGate) {
  // Stats gate is ADVISORY ONLY — never overrides guardian state to RED.
  // It reduces risk multiplier and adds warnings, but won't block execution.
  // This prevents stale historical stats (old demo trades, previous attempts)
  // from locking out a fresh challenge.
  const merged = { ...guardianState };

  // Add stats warnings for visibility, but NOT as blockers
  merged.warnings = [...(merged.warnings || []), ...statsGate.warnings];
  merged.stats_gate = statsGate.gate;
  merged.risk_multiplier = statsGate.riskMultiplier;
  merged.stats_detail = statsGate.detail;
