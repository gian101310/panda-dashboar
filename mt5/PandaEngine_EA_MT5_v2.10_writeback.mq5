...
struct PandaSignal
{
   ...
   string   boxH4;
   int      confluence;
   string   writeTime;     // UTC timestamp from engine score file (WRITE_TIME field)
   bool     valid;
};
struct SRLevel
{
...
struct EntryRecord
{
   ...
   ulong    magic;
   int      spreadAtEntry;
   string   signalWriteTime;  // from score file WRITE_TIME field
   string   eaReadTime;       // when EA read the score file
};

EntryRecord g_entries[];
int         g_entryCount = 0;

// Latency tracking — set on each ReadPandaSignal(), consumed by RecordEntry()
string g_lastSignalWriteTime = "";
string g_lastEaReadTime      = "";
...
void RecordEntry(ulong orderTicket, double requestedPrice, ulong magic)
{
   ...
   g_entries[g_entryCount].signalWriteTime = g_lastSignalWriteTime;
   g_entries[g_entryCount].eaReadTime = g_lastEaReadTime;
   g_entryCount++;
                      ...
                      double profitMoney, double lotSize)
{
   ...
   // Latency tracking — include signal_write_time and ea_read_time if available
   EntryRecord latRec;
   if(FindEntry(posId, latRec))
   {
      if(StringLen(latRec.signalWriteTime) > 0)
         json += ",\"signal_write_time\":\"" + JE(latRec.signalWriteTime) + "\"";
      if(StringLen(latRec.eaReadTime) > 0)
         json += ",\"ea_read_time\":\"" + JE(latRec.eaReadTime) + "\"";
   }
...
bool ReadPandaSignal(string symbol, PandaSignal &sig)
{
   ...
   // --- FILE-AGE CHECK: skip signals older than 5 minutes ---
   datetime fileModTime = (datetime)FileGetInteger(handle, FILE_MODIFY_DATE);
   datetime now = TimeCurrent();
   int fileAgeSec = (int)(now - fileModTime);
   if(fileAgeSec > 300)
   {
      PrintFormat("[PANDA] Signal file too old (%d min), skipping %s", fileAgeSec / 60, filename);
      FileClose(handle);
      sig.valid = false;
      return false;
   }
   ...
   while(!FileIsEnding(handle))
   {
      ...
      else if(key == "WRITE_TIME")  sig.writeTime   = value;
   }
   FileClose(handle);
   sig.valid = true;

   // Capture latency timestamps for write-back
   g_lastSignalWriteTime = sig.writeTime;
   g_lastEaReadTime = DateTimeToISO(TimeCurrent());
